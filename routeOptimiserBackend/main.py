...
def find_routes():
    try:
        ...
        # Fallback: if Nominatim string matching fails, grab the physically closest nodes
        if not initial_nodes:
            logger.info("Country string match failed for start. Falling back to proximity search.")
            distances = []
            for n, data in G.nodes(data=True):
                if "latitude" in data and data["type"] in ["seaport", "airport"]:
                    from src.utils.geocoding import GeocodingUtils
                    dist = GeocodingUtils().haversine_distance(start_coords, (data["latitude"], data["longitude"]))
                    distances.append((dist, n))
            # Grab top 5 closest
            distances.sort(key=lambda x: x[0])
            initial_nodes = [n for _, n in distances[:5]]
        ...
        if not final_nodes:
            logger.info("Country string match failed for end. Falling back to proximity search.")
            distances = []
            for n, data in G.nodes(data=True):
                if "latitude" in data and data["type"] in ["seaport", "airport"]:
                    from src.utils.geocoding import GeocodingUtils
                    dist = GeocodingUtils().haversine_distance(end_coords, (data["latitude"], data["longitude"]))
                    distances.append((dist, n))
            distances.sort(key=lambda x: x[0])
            final_nodes = [n for _, n in distances[:5]]
